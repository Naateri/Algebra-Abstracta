...
string ElGamal::cifrar(string msj){
    ...
    string leng = zToString(this->p), c, original, tmp, alfLeng = zToString(NTL::to_ZZ(alfL));
    ...
    for (i = 0; i < msj.size(); i++){ ///llenando los bloques con 2 digitos (tam de la letra mas grande)
        ...
        ///guardamos ese numero en un string*/
    }
...
string ElGamal::descifra_mensaje(string c){
    long alfL = this->alfabeto.size();
    string leng = zToString(this->p), d, ret, tmp, c1, alfLeng = zToString(NTL::to_ZZ(alfL));
